    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        async function loadCallState() {
            ...
            try {
                ...
                updateSystemStatus("🟢 In Call (Restored)", "status-in-call");
                // renderProgressTracker() will be called by renderStep()
                renderStep(); // This will render the step and call startStepTimer

                showAssistantMessage(`🚀 Call restored to "${currentScenarioName}", step ${currentStepIndex + 1}.`, true, 7000);
                return true;
            } catch (e) {
            ...
            if (!callWasRestored) {
                ...
                showAssistantMessage("Welcome! Select a scenario to begin.", true, 6000, () => {
                    if (initialViewDiv && initialViewDiv.style.display === 'block') {
                        showAssistantMessage("💡 Tip: Choose a scenario and click 'Receive Call'.", true, 0);
                    }
                });
            } else {
        ...
        if (receiveCallBtn) {
            receiveCallBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    showAssistantMessage(`🚀 Scenario "${currentScenarioName}" started! Follow the steps.`, true, 0);
        ...
        if (endCallBtn) {
            endCallBtn.addEventListener('click', async () => {
                stopAndRecordStepTimer(); 
                const totalDuration = stepDurations.reduce((acc, duration) => acc + (duration || 0), 0);
                updateSystemStatus("⚪ Call Ended Early", "status-ended"); 
                if (callTimerDiv) callTimerDiv.style.display = 'none';
        ...
        function displayPostCallSummary(quality, reason, totalDuration) {
            ...
            updateSystemStatus("📊 Review Call Summary", "status-summary"); 
        }
...
// ✅ تم إضافة هذا الجزء لتقييم المكالمة عند الانتهاء
async function endCall() {
  const sessionData = {
    stepsCompleted: completedSteps,
    totalSteps: scenarioSteps.length,
    callQuality: completedSteps === scenarioSteps.length ? "Good" : (completedSteps >= scenarioSteps.length / 2 ? "Normal" : "Bad"),
    reason: completedSteps === scenarioSteps.length ? "-" : "Didn't complete all steps",
  };

  const result = await evaluateCallSession(sessionData);
  alert(`Call Evaluation: ${result.quality} - ${result.reason}`);
}
